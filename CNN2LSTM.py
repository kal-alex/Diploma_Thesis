...
# reshape from [samples, window, rows, columns] into [samples, rows, columns, window]
...
# Define model
model = Sequential()
model.add(Conv2D(filters=64, kernel_size=5, activation='tanh', padding="same", input_shape=(NODES,NODES,WINDOW_IN)))
model.add(Conv2D(filters=64, kernel_size=3, activation='tanh', padding="same"))
